...
function WhiteScreen.ToggleWhiteScreen(Value)
    ...
    if Value then
        if not ExistingGui then
            ...
            ScreenGui.DisplayOrder = -100 -- Ensure it is behind the GUI (which is usually 0)
            ...
            Frame.ZIndex = 1 -- Adjusted ZIndex
            Frame.Parent = ScreenGui

            -- Optional: Add a label so the user knows the script is still running
